document.addEventListener('DOMContentLoaded', () => {
    ...
    let redPlayerSocketId = null;
    let bluePlayerSocketId = null;
    let gameStarted = false;

    io.on('connection', (socket) => {
        // Assign players to red and blue teams
        if (!redPlayerSocketId) {
            redPlayerSocketId = socket.id;
        } else if (!bluePlayerSocketId) {
            bluePlayerSocketId = socket.id;
        }
    
        socket.on('startGame', () => {
            // Check if both players are connected and the game hasn't started yet
            if (redPlayerSocketId && bluePlayerSocketId && !gameStarted) {
                gameStarted = true;
                const { width, totalMines } = DIFFICULTY_SETTINGS;
                const board = createBoard(width, totalMines);
                // Emit event to start the game and send the initial board state to both players
                io.emit('gameStarted', { board });
            }
        });
    
        socket.on('revealTile', ({ tile, playerId }) => {
            // Check if it's the player's turn and reveal the tile
            if (gameStarted && (playerId === redPlayerSocketId || playerId === bluePlayerSocketId)) {
                revealTile(tile, playerId);
            }
        });
    
        socket.on('flagTile', ({ tile }) => {
            // Flag the tile
            flagTile(tile);
        });
        ...
        socket.on('disconnect', () => {
            ...
            if (socket.id === redPlayerSocketId) {
                redPlayerSocketId = null;
            } else if (socket.id === bluePlayerSocketId) {
                bluePlayerSocketId = null;
            }
            // Reset the game if both players are disconnected
            if (!redPlayerSocketId && !bluePlayerSocketId) {
                gameStarted = false;
            }
        });
    });
